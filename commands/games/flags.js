  ...
  async executeClassic({message, args}, { client, embedColors, db }) { 
    ...
    switch (args[0]) { // Switch to handle different subcommands.
      case 'quiz': { //MARK: quiz subcommand
        ...
        //send embed
        const response = await message.reply({embeds: [flagEmbed], components: buttonRowArray})
        ...
        //initialize arrays of correct and wrong users; used for point changes later
        let correctUsers = [];
        let wrongUsers = [message.author.id]; //automatically a loser, just in case you don't answer
      ...
      case "stats": { //MARK: stats subcommand
        ...
        try {
          ...
          return await message.reply("couldn't find that user, ping them or use their id")
        }
